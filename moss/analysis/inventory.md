...
深入范围现覆盖完整桌面对话工作台：App shell、展开/折叠侧栏、Agent 切换、分组会话列表、active/unread/generating/rename 状态、会话 header、消息流、用户/assistant 消息、思维链/工具动作、Markdown 正文、输入框、右侧文件抽屉、scope/search/upload/batch/grid/list/preview/loading；以及右侧智能看板：drawer 壳、tab、查询表单、流式反馈、loading/empty/error、iframe 渲染（见 `boards.md`）。2026-08-06 起前端**非测试源码全量归档**：共享组件（`common-components.md`）、管理控制台（`admin-console.md`）、认证/公开/外围页（`auth-public-pages.md`）、Studio 与功能模块（`studio-modules.md`）均完成区域级分析。
