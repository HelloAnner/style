# Light 主题

## 核心语义

| 语义 | 值 |
|---|---|
| canvas | `#FAF9F7` |
| chat main | `#FCFCFB` |
| secondary surface | `#FFFFFF` |
| tertiary surface | `#F5F4F2` |
| text primary / secondary / tertiary | `#1A1A1A` / `#3A3A3A` / `#5A5A5A` |
| muted / disabled | `#7A7A7A` / `#C4C4C7` |
| border subtle / default | `rgba(0,0,0,.06)` / `#E4E4E7` |
| user bubble | `#F0EFED` |
| agent bubble | `#FFFFFF` |
| brand / send | `#D95E3A` / `#DE6A43` |
| file/link | `#2563EB` |

`Observed · exact-source · high · light · SRC-001`

思维链在暖米白 canvas 上直接排版；完成圆勾和主过程文字使用 secondary，工具动作退到 tertiary，耗时与运行标题使用 muted。正文才使用白色 surface。

运行态产品当前固定此主题。参考实现截图：`EVD-001`、`EVD-003`，但不是来源产品 baseline。
