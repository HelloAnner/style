...
**当前档案尚处于 intake，未提供任何风格证据。实时状态不是 `complete` 时，禁止声称已经参考或可以完全复刻 wufan。**
...
上述文件当前大多尚未建立，这是已登记的 blocker，不得跳过。只有 manifest 为 `complete` 且目标范围已 validated，才能无条件声称严格复刻。否则先向用户说明相关缺口；用户允许带缺口继续后，仍需记录推断和偏差。
